---
title: "钱包交易所集成"
slug: "integrations-wallet-exchange-integration"
locale: "zh"
category: "integrations"
source_url: "https://docs.canton.network/integrations/wallet/exchange-integration.md"
source_title: "Exchange Integration Guide"
tags:
  - integrations
  - wallet
...
> Integrate an exchange with Canton Network for Token Standard deposits and withdrawals.

# Exchange 集成指南

The pages below guide you how to integrate an exchange with Canton for the purpose of trading Canton Network Token Standard compliant tokens like Canton Coin（CC）.

The core of such an 集成 is automating the deposits and withdrawals of tokens to and from the exchange. The guide can 因此 also be used for 服务 除…之外 exchanges that want to support deposits and withdrawals of Canton Network tokens.

* [概览](#overview)
* [架构](#architecture)
* [工作流](#工作流)
* [交易 Ingestion](#交易-ingestion)
...
本指南 旨在 to help you integrate your exchange with Canton for the purpose of trading Canton Coin（CC） (CC) and Canton Network (CN) tokens.
...
<img src="https://mintlify.s3.us-west-1.amazonaws.com/cantonfoundation/集成/钱包/images/component_diagram.png" alt="集成 architecture 组件 overview" />
...
<img src="https://mintlify.s3.us-west-1.amazonaws.com/cantonfoundation/集成/钱包/images/1-step_deposit.png" alt="1-Step 充值 工作流 Diagram" />
...
<img src="https://mintlify.s3.us-west-1.amazonaws.com/cantonfoundation/集成/钱包/images/1-step_withdrawal.png" alt="1-Step 提现 工作流 Diagram" />
...
* Canton Coin（CC） charges a small holding fee of about \$1 per year for each `Holding` UTXO to allow archiving [dust coins](https://www.investopedia.com/terms/b/bitcoin-dust.asp) once their holding fee surpasses their value.
...
<img src="https://mintlify.s3.us-west-1.amazonaws.com/cantonfoundation/集成/钱包/images/multi-step_deposit.png" alt="Multi-Step 充值 工作流 Diagram" />
...
<img src="https://mintlify.s3.us-west-1.amazonaws.com/cantonfoundation/集成/钱包/images/multi-step_withdrawal.png" alt="Multi-Step 提现 工作流" />
...
例如, Canton Coin（CC） also exist on TestNet and DevNet with different `dsoParty` ids. 你可以 retrieve the `dsoParty` id for each 网络 using the CC Scan API served from the [SV nodes of that 网络](https://sync.global/sv-网络/):
...
你可以 parse such 交易 using the [token standard history parser](https://github.com/canton-网络/钱包/blob/main/core/ledger-client/src/txparse/parser.ts) provided in the 钱包 SDK to extract the deposit amount, 账户 and holding 合约 ids. 若你 prefer implementing your own implementation, you can parse this as follows:
...
For Canton Coin（CC）, both the creation of the `TransferInstruction` as well as the acceptance currently charge fees so the amount specified in the transfer is smaller than the 持仓 change of the treasury party. Once the [CIP for CC fee removal](https://lists.sync.global/g/cip-discuss/message/304?p=%2C%2C%2C20%2C0%2C0%2C0%3A%3Arecentpostdate%2Fsticky%2C%2Cfee%2C20%2C1%2C0%2C115127097) is implemented, this distinction goes away. Currently Canton Coin（CC） is the only token on Canton Network charging such fees.
...
2. Copy the party id of your `exchangeParty（交易所 Party）` from the Splice 钱包 UI as explained above, or retrieve it by calling `/v0/validator-用户` on the [验证者 API](https://github.com/hyperledger-labs/splice/blob/36ed55ea1fbb9b0030000bb0d0265ba811101df5/apps/validator/src/main/openapi/validator-internal.yaml#L14C3-L14C21).

3. Call `/v2/命令/submit-and-wait` on the [Ledger API](https://github.com/digital-asset/canton/blob/eeb56bc5d9779a7f918893b7a6b15e0b312a044e/community/ledger/ledger-json-api/src/test/resources/json-api-docs/openapi.yaml#L6C3-L6C31) to create a `#splice-wallet:Splice.钱包.TransferPreapproval:TransferPreapprovalProposal` ([code](https://github.com/hyperledger-labs/splice/blob/edb2257410dfc3660314765c40e59f41e2381150/daml/splice-wallet/daml/Splice/钱包/TransferPreapproval.daml#L9)) directly with the `提供方` set to your `exchangeParty（交易所 Party）`.
...
1. 下载 the most recent version of the `splice-util-featured-app-proxies.dar` file from the [Splice repository's checked-in .dars](https://github.com/hyperledger-labs/splice/tree/main/daml/dars).
...
4. 添加 a step to the treasury party setup to also create a `DelegateProxy` 合约 with `提供方 = exchangeParty（交易所 Party）` and `delegate = treasuryParty（金库 Party）`.
   ...
   Use the `/v2/命令/submit-and-wait` [端点](https://github.com/digital-asset/canton/blob/97b837d7b7e9a499963cba1d39a017648c46e8d7/community/ledger/ledger-json-api/src/test/resources/json-api-docs/openapi.yaml#L6) submit the `create` 命令 for the `DelegateProxy` template.
...
<div className="todo">
  [https://github.com/canton-网络/钱包/issues/272](https://github.com/canton-网络/钱包/issues/272) 更新 this when 钱包 SDK support is available
</div>
...
Until then, the easiest way to do so at the moment is through the Canton console. 你可以 find a full reference for all required steps in the [集成 test](https://github.com/digital-asset/canton/blob/3c9ac9891c03cb06303736d7224bcc01dbd50084/community/app/src/test/scala/com/digitalasset/canton/集成/tests/jsonapi/ExternalPartyLedgerApiOnboardingTest.scala#L183). Note in particular that you must sign the `PartyToParticipant` mapping not just by your party's key but also by all confirming participants. This is accomplished through the `participant2.topology.交易.authorize` step in the test.
