...
该页面指定了智能合约共识层的验证逻辑、确认策略和安全属性。有关交易排序和排序器机制，请参阅[排序共识](/zh/docs/canton/overview-reference-ordering-consensus)。有关端到端消息流，请参阅[事务生命周期](/zh/docs/canton/overview-reference-transaction-lifecycle)。
...
排序层（排序器和中介器）处理全局一致性——防止双花并建立状态更改的总顺序。智能合约共识层负责处理正确性——确保 Daml 逻辑得到忠实执行并满足授权规则。这种分离就是[Canton协议规范](/zh/docs/canton/overview-reference-canton-protocol-specification) 所说的两层共识。
...
**原子性。** 调解员的裁决是全有或全无。事务中的每个操作要么都提交，要么都不提交。没有部分提交状态。**活跃性。** 交易进度取决于排序器（用于消息排序和分发）、调解器（用于判决聚合）和足够多的确认参与者（以满足每个签名者的确认阈值）的可用性。如果这些组件中的任何一个在协议超时后不可用，则事务将中止。排序层的活跃度在[排序共识](/zh/docs/canton/overview-reference-ordering-consensus)中进行了分析。
