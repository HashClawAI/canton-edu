---
title: "Wallet SDK"
slug: "integrations-release-notes-wallet-sdk"
locale: "zh"
category: "integrations"
source_url: "https://docs.canton.network/integrations/release-notes/wallet-sdk.md"
source_title: "Wallet SDK"
tags:
  - integrations
  - release-notes
  - wallet-sdk
...
> Release notes for the Canton Network 钱包 SDK

以下为 钱包 SDK 各版本的发布说明 for the 钱包 SDK versions, detailing new features, improvements, and bug fixes in each version.

## 1.1.0

**发布日期： April 28th, 2026**
...
**发布日期： April 17th, 2026**

钱包 SDK v1 is the new long-term API shape for 应用 开发. Rather than being an additive patch to v0, v1 introduces a cleaner and more explicit model focused on composability, multi-party flows, and transport flexibility.
...
* Clearer 交易 lifecycle: flows are modeled as explicit `prepare -> sign -> execute` steps, improving observability and reducing hidden behavior.

* Better API organization: v0 controllers are replaced by namespaces (`ledger`, `party`, `token`, `amulet`, `用户`, `asset`, `事件`)
  with clearer boundaries and easier discoverability.

* More flexible 集成 model: v1 supports static 配置 and 提供方-based initialization, making it easier to integrate with browser,
  dApp, remote 钱包, and alternative transport setups.
...
This release note intentionally avoids listing every single 方法-level change. For full migration details and examples, use the migration guide:

* [钱包 SDK v1 Migration Guide](https://github.com/canton-网络/钱包-gateway/blob/82ec39c9/docs/wallet-integration-guide/src/钱包-sdk-v1-migration-guide/index.rst)
* [v1 Ledger Migration](https://github.com/canton-网络/钱包-gateway/blob/82ec39c9/docs/wallet-integration-guide/src/钱包-sdk-v1-migration-guide/ledger.rst)
* [v1 Token Migration](https://github.com/canton-网络/钱包-gateway/blob/82ec39c9/docs/wallet-integration-guide/src/钱包-sdk-v1-migration-guide/token.rst)
* [v1 Party Migration](https://github.com/canton-网络/钱包-gateway/blob/82ec39c9/docs/wallet-integration-guide/src/钱包-sdk-v1-migration-guide/party.rst)
* [v1 用户 Migration](https://github.com/canton-网络/钱包-gateway/blob/82ec39c9/docs/wallet-integration-guide/src/钱包-sdk-v1-migration-guide/用户.rst)

## 0.21.1

**发布日期： February 20th, 2026**

* fix fetching active 合约 in loop mode

*a bug was identified where fetching active 合约 in loop mode (`continueUntilCompletion`) would not correctly handle pagination, resulting in incomplete results being returned. This has now been fixed ensuring all 合约 are reliably returned.*

## 0.21.0

**发布日期： February 6th, 2026**

* pagination support for listing UTXOs

*previously `listHoldingsUtxo` was limited to the ledger API upper bound of 200 items per 请求. For Party with a large number of 持仓 this meant not all UTXOs would be returned. A new optional `continueUntilCompletion` parameter has been added that when set to true will automatically page through all results, making it possible to reliably work with Party that hold more than 200 UTXOs.*
...
*when calling `prepareSubmission` against a Canton 3.4 参与者 the `costEstimation` field was previously dropped from the 响应. The 响应 type has been corrected and the cost estimation is now correctly surfaced.*
...
**发布日期： January 16th, 2026**

* subscribe to ledger update 事件 via WebSocket

*it is now possible to open a persistent WebSocket connection and receive a real-time stream of ledger update 事件 filtered by interface or template id. The stream is exposed as an async generator so it integrates naturally with `for await` loops.*
...
* subscribe to 命令 completions via WebSocket

*similarly to update subscriptions, you can now subscribe to 命令 completion 事件 so you can react to completed or failed 命令 in real time instead of polling.*
...
* `listWallets` now only returns local Party

*previously `listWallets` could return Party that the 用户 had been granted access to on remote validators, this was confusing and incorrect behaviour. The 方法 now only returns Party that are locally allocated on the connected 参与者.*
...
* **重要!: LedgerController constructor has changed to named parameters**

*the `LedgerController` constructor has been refactored from positional parameters to a named parameter object. This is a breaking change if you construct `LedgerController` directly. The new signature also accepts an optional custom `fetch` implementation which is useful for routing requests through an intermediary such as the 钱包 gateway.*
...
* get created 合约 by update id

*a new 方法 `getCreatedContractByUpdateId` has been added on the ledger controller. After submitting a 交易 you can use the returned `updateId` to look up the 合约(s) that were created as part of that 交易. Optionally you can narrow the result by providing template or interface ids.*
...
*the logic for Accept, Reject and Withdraw on a transfer instruction has been consolidated into a single `createTransferInstruction` 方法, reducing boilerplate when you want to exercise a choice without caring about which specific one.*
...
*the `@canton-网络/core-ledger-client` package can now be imported and used directly in a browser environment. Node.js-specific modules have been removed from the main bundle so that browser-based dApps and portfolio UIs can leverage the ledger utilities without additional bundler workarounds.*

* fixed decimal precision handling

*decimal arithmetic is now handled using `decimal.js` to prevent floating-point precision 错误 when working with large or fractional CC amounts.*

## 0.18.0

**发布日期： November 26th, 2025**

* merge utxos 命令

*you can now easily perform utxos merging for you by simply calling the 方法 `mergeHoldingUtxos`, this returns a series of 命令 that each needs to be executed individually to merge the assets. It returns a list of utxos 命令 because: Firstly, it supports multi-assets (so it will merge both CC and non-CC tokens) and secondly there is an upper limit of 100 inputs per 交易 so to facilitate if more is present then it splices it for the client.*
...
*in previous release we enabled the purchase of 流量 using an external party, now we have included an option to fetch the 余额 so you can check if you actually need to top up with an external party. However this does require providing a scanApiBaseUrl to the tokenStandardController.*
...
*listHoldingsUtxo have been extended with an optional 偏移 parameter, normally it uses the ledgerEnd, but this allow you to define it yourself. Do be warned that this is not a performant operation.*

## 0.17.0

**发布日期： November 14th, 2025**

* 钱包 SDK has been updated to support 0.5.1 & Canton 3.4.7
...
* 钱包.localhost changed to localhost
...
*previously when perform a transfer and not providing utxos (as an empty array), then the sdk would automatically select all utxos to perform the transfer. This had various problems like adding more than 100 utxos or utxos not having enough funds, this has also been improved with better 错误 messaging.*

* Better automation around token metadata

*the token standard controller have two new 方法: `getInstrumentById` & `listInstruments`, these uses the transferRegistryUrl provided to fetch relevant data from the original source making non-CC token 集成 more seamless. Likewise in certain cases the instrumentAdmin has been made optional since we can use the above to fetch these.*

* test improvements of snippets

*test snippets used as part of the 钱包 集成 guide was previously considered theoretical examples, the entire suite has been upgraded and now each snippets have been tested against a running validator to ensure it is correct in its completeness. This means we also had to change some of the values from using example naming the actual naming.*

* buy member 流量

*new 方法 added that allows the purchase of 流量 for a specific validator using an external party.*
...
*toDecodedTopologyTransaction introduced in previous version has been moved as a static 方法 on the ledgerController, this is primarily so it can be used in offline mode.*
...
**发布日期： November 4th, 2025**

* caching for amulet rules and open mining round

*Amulet rules are mostly static and open mining round persists for 10 min, so we can cache this 响应*
...
**发布日期： October 29th, 2025**

<警告>
  Due to CommonJS compatibility, the module type needs to be explicitly declared in certain cases. 若你 see an 错误 like `错误: Top-level await is currently not supported with the "cjs" output format` and are intended on using ESM then you should set `"type": "module"` in your package.json.
</警告>
...
*Querying the ACS is an expensive ledger API operation, as an alternative the ACS is fetched into memory once and subscribe to new 事件 instead. This should significantly reduce the load on the ledger especially in heavy read operations.*

* Caching of Access Tokens

*Previously a new token was retrieved from the AuthController every time a 请求 was made, this is not a huge problem for `unsafe` tokens, however still unnecessary. Tokens are now kept in memory and reused and a new token is only requested upon expiry.*
...
* **重要!: await completion has changed signature**
...
*this change is to make it simpler, the 方法 would regardless throw an 错误 if commandId and SubmissionId was undefined*

* retry logic & stress 测试

*we have substantially tested and verified the SDK working on moderate and heavy load mimicking MainNet, this highlighted some retryable 错误 that could be handled.*

* multi hosted party fix and synchronized handling

*multi hosted Party have had a change under the hood, previously it would return the party asynchronously. This has been resolved by calling the allocation on all the available ledger thereby ensuring the party is ready for use once it is returned by the 方法.*
...
**发布日期： October 15th, 2025**

* **重要!: The custom meta-data on create transfer have changed format**
...
*The 钱包 SDK is heavy focused on external party submission flows, however there are certain administrative tasks that requires using the validator operator party (which is internally hosted). This is especially useful for setting up delegation proxy 合约 needed for featured app markers.*
...
* Possibility to create 命令 offline

*certain 命令 like transfer required to be performed in an online context, this was needed to fetch relevant data like transferInstruction choice context. This 方法 (and allocation) have now been extended with optional parameters in case that it is preferred to be pre-fetched.*
...
*TLS 配置 can now be provided for the topologyController allowing a safe and secure connection.*
...
*Party creation is under heavy load on mainnet and would consistently run into: `The server was not able to produce a timely 响应 to your 请求`. Safe guard has been added against this, when the 错误 occurs we continuously look for the party to be available. If a timeout is required then it will have to be handled outside of the 方法. It is worth nothing that the party creation has no timeout on ledger.*
...
**发布日期： October 10th, 2025**

* Added support to tap internal Party

*previously you could only tap external Party using signing flow, now it can be done for internal Party. this is useful for tapping the validator operator party right after startup in case of missing funds.*
...
*the functionality have been added for the adminLedgerController to upload dars, this is useful for 测试 custom dar flows*
...
the multi-hosted 配置 is the same, except that **the ledger you call** should not be included in the array
...
for backwards compatibility the previous 端点 are still there and available.

* 用户 creation and rights management

*you can now create new 用户 and manage rights through the 钱包 SDK. This can be useful for setting up a master 用户*
...
* ListWallets now correctly returns the Party that the 用户 has access to (including CanReadAsAnyParty)
* Extended the max timeout when onboarding a party from 20s to 1 minute
* Party onboarding now queries the specific party instead of all Party (performance improvement)
* Party onboarding now has idempotent behavior
* Default values changed for 钱包 SDK from `localLedgerDefault` to `localNetledgerDefault` on all controllers
...
**发布日期： October 2nd, 2025**

* Self-issue feature app rights

*you can now grant yourself feature app rights (similar to the 钱包 UI) for both internal and external Party*
...
* topology 交易 recalculate hash

*you can now offline validate a topology 交易 by recomputing the hash*
...
* `prepareSubmission` now has same 命令 input signature as `prepareSignAndExecuteTransaction`

## 0.9.0

**发布日期： September 26th, 2025**
...
*since the 钱包 api and registry are static for localnet, a new config has been added to make early 开发 easier*
...
* **重要!: The flow has been simplified for prepare and execute of 命令, however this means code needs to be converted**
...
this goes for all 交易!

* Support 提现 flow for 2-step transfer
...
* Allow validating if receiver have set up transfer pre-approval before performing a 交易
...
* **重要!: scan api is not longer used for 方法 like `connectTopology` use scan proxy instead**
...
* Added new 端点 to quickly fetch all pending 2-step incoming transfer to easily accept or reject
...
* ledgerFactory, TopologyFactory & 验证者Factory changed to use URL instead of strings (where applicable)
...
* stronger typing now required strings of specific formats for Party across all controllers
...
* Include some small bug fixes. The most noteable are:
  * `合约 not found` 错误 when listing 持仓 ([https://github.com/canton-网络/钱包-gateway/issues/357](https://github.com/canton-网络/钱包-gateway/issues/357))
