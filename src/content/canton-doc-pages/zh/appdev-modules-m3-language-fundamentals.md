---
title: "语言基础"
slug: "appdev-modules-m3-language-fundamentals"
locale: "zh"
category: "appdev"
source_url: "https://docs.canton.network/appdev/modules/m3-language-fundamentals.md"
source_title: "Language Fundamentals"
tags:
  - appdev
  - modules
...
在 `contracts` 中你学习了合约模板——定义可在账本上创建的合约类型及其参数数据。

在 `daml-scripts` 中你学习了 Daml Studio 的 script 视图，显示当前账本状态：每个模板一表，每合约一行，每字段一列。

可将模板类比为数据库表：模板为账本定义数据模式：

* 每个模板对应一张表
* `with` 块中每个字段对应一列
* 每个该类型合约对应一行

本节学习如何为账本设计丰富数据模式，具体包括：
...
学完后，你应能把 Daml 账本当作简单数据库，各方可写入、读取、删除复杂数据。

<Tip>
  运行 `dpm new intro-data --template daml-intro-data` 可将本节全部代码加载到 `intro-data` 文件夹。
</Tip>

## 原生类型

你已见过部分原生类型：`contracts` 中的 `Party`，`daml-scripts` 中的 `Text`、`ContractId`。更多如下：

* `Party` 存储能在账本上行动的主体身份（签署合约、提交交易等）。通常 `Party` 不透明。
* `Text` 存储 Unicode 字符串，如 `"Alice"`。
* `ContractId a` 存储类型 `a` 合约的引用。
* `Int` 有符号 64 位整数，如 `-123`。
* `Decimal` 定点数：小数点前 28 位、后 10 位。
* `Bool` 为 `True` 或 `False`。
* `Date` 日期。
* `Time` 绝对 UTC 时间。
* `RelTime` 时间差。

下列 script 实例化各类型、在适当时操作并测试结果：
...
* 顶部 `import` 从标准库引入日期时间与 Daml Scripts 函数。包、导入与标准库后文再述。

* 多数变量在 `let` 块中声明，因 `script do` 块期望 `submit`、`allocateParty` 等 script 动作；`123` 等是纯表达式，可在无账本情况下求值。`let` 将变量声明变为动作。

* 多数变量无类型注解，因 Daml 擅长**推断**。`my_int = 123` 可推断 `my_int : Int`。若类型歧义则须注解，如 `0.001` 可为任意 `Numeric n`，须写 `0.001 : Decimal`（`Numeric 10` 的别名）。可随时加注解以提高可读性。

* `assert` 为接受布尔值的动作，`True` 成功、`False` 失败。在 script 中放 `assert False` 观察结果。

用模板与这些原生类型已可写出类似关系库表的模式。下面将 `Token` 扩展为由会计 Party 管理的简单 `CashBalance`：
...
`CashBalance` 上信息较多，宜赋予更多结构。Daml 类型系统提供多种将原生类型组合为更丰富结构的方式。

### 元组

常见需求是以通用方式分组，例如 `Text` 键与 `Int` 值的键值对可用 `(Text, Int)` 二元组；三维坐标可用 `(Decimal, Decimal, Decimal)` 三元组：
...
* 函数 `fst`、`snd`、`fst3`、`snd3`、`thd3`
* 点语法字段 `_1`、`_2`、`_3` 等

Daml 支持最多 20 元素元组，但 `fst` 等访问器仅内置给 2、3 元组。

### 列表

列表带单类型参数。可有 `[Int]` 或 `[Text]`，不能混放整数与字符串——Daml 静态强类型，取元素时编译器须知类型。

下列 script 实例化整数列表并演示重要列表函数：
...
注意 `empty : [Int] = []` 的注解：`[]` 歧义，可能是整数或字符串列表。

### 记录

记录可视为带名字段的元组。用 `data` 声明：`data T = C with`，`T` 为类型名、`C` 为数据构造子。实践中类型名与构造子同名较好：
...
声明语法与模板很像，因模板本质是特殊记录：`template Token with` 在底层会变为 `data Token = Token with`。

上文 `assert` 比较内置类型。若写 `assert (my_record == my_record)` 可能报错 `No instance for (Eq MyRecord)`——Daml 相等为值相等，须为 `MyRecord` 实现相等；可用 `deriving (Eq)` 让编译器自动生成：
...
`Eq` 是**类型类**，类似其他语言的接口：定义一组函数（如 `==`、`/=`）适用于多种类型。编译器还可自动派生 `Show`（`show` 函数）与 `Ord`（`<`、`>` 等）。

建议始终 `deriving (Eq, Show)`。`template T with` 创建的记录会自动派生；原生类型亦有实例，如 `Int` 派生 `Eq`、`Show`、`Ord`；`ContractId a` 派生 `Eq`、`Show`。

记录可为 `CashBalance` 数据赋予结构：
...
script 视图中仍为一表，记录展开为带点记法的列。

### 变体与模式匹配

若还要记录手头现金：手头现金无银行，但不能将 `bank` 留空——Daml 无 `null` 等价物。变体可表达现金或在手或在银行：
...
`Location` 的读法：「`Location` **要么**为 `InHand`，**要么**为 `InAccount a`，其中 `a` 为 `Account` 类型」。明确表达 `CashBalance` 是否关联 `Account`，并为两种情况命名。

也可用内置 `Optional`：`Optional a` 的 `None` 最接近 `null`：
...
访问变体数据须区分情况；`InHand` 时可能无账号，不能直接取账号。用**模式匹配**，可抛错或为各情况返回兼容类型：
...
你已具备在账本上表达丰富类型的要素，并知道如何创建与读字段。值一旦创建如何「修改」？

Daml 数据**不可变**：创建后永不改变，只能基于旧值创建新值并应用变更：
...
`changed_record` 与 `better_changed_record` 均为 `eq_record` 的副本并修改 `my_int`。推荐写法：记录名换为旧值 `eq_record with`，`with` 块不必列出全部字段，缺失字段取自 `eq_record`。

全程 `eq_record` 不变。`"Zero" :: eq_record.my_list` 非就地修改列表，而是创建以 `eq_record.my_list` 为前缀的新列表。

## 操作合约

Daml 类型系统允许在合约中存储结构化数据。与记录等一样，合约不可变，只能创建与归档。要「改」合约须归档原合约并创建含新数据的新合约：
...
上述 script 在同一交易用 `archiveCmd` 与 `createCmd` 修改 `Account` 合约，产生新合约与新 contract ID，故 `newAccountCid =/= accountCid`。

归档再创建会使 contract ID 不稳定，可能导致**陈旧引用**：
...
`balance.account` 中的 `ContractId` 仍指向已归档合约，查询失败返回 `None`。

## 下一步

你已能为账本定义数据模式并读写删除数据。在 `choices` 中将学习如何定义数据变换并授权其他 Party 以受限方式操作数据。
