---
title: "Daml 错误码"
slug: "appdev-troubleshooting-guide-error-code-reference"
locale: "zh"
category: "appdev"
source_url: "https://docs.canton.network/appdev/troubleshooting-guide/error-code-reference.md"
source_title: "Daml Error Codes"
tags:
  - appdev
...
**修复：** 检查您正在调用的函数的类型签名。在Daml脚本中使用`fetch`将`ContractId`转换为其有效负载，或使用`getParty`将`Text`转换为`Party`。
...
**原因：** 您试图创建一个密钥与已激活的合约相匹配的合约。 Daml 在参与方的可见性内强制每个模板的关键唯一性。**修复：** 在创建之前检查具有相同密钥的现有合约。在Daml中使用`lookupByKey`或通过Ledger API查询ACS。如果您想替换现有合同，请先将其存档。
