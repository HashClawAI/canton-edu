---
title: "钱包集成指南"
slug: "integrations-wallet-guidance"
locale: "zh"
category: "integrations"
source_url: "https://docs.canton.network/integrations/wallet/guidance.md"
source_title: "Wallet Integration Guide"
tags:
  - integrations
  - wallet
  - guidance
...
<div id="implications-for-钱包-提供方">
  **Implications for 钱包 提供方**
</div>

To offer 服务 on the Canton Network, you will need a **验证者节点 to host your Party and your customers' Party**. You have two options for this: you can **self-host** a node or use a [node-as-a-服务 提供方](https://sync.global/current-validators-offering-nodes/).
...
* Support the [CIP-0056 token standard](https://github.com/canton-foundation/cips/blob/main/cip-0056/cip-0056.md) to enable the holding and transferring of assets on the Canton Network. Documentation and guidance on how to implement this with the 钱包 SDK is in the Token Standard section of this guide.
...
The 钱包 SDK uses a 交易 parsing transform a fully fledged 交易 tree into human recognizable 交易 view. The full code for the 交易 parsing can be found at [parser typescript class](https://github.com/canton-网络/钱包/blob/main/core/ledger-client/src/txparse/parser.ts).
...
1. **Creating a 命令** - You start by simply creating a 命令.
2. **Preparing the 交易** - You send the 命令 to the blockchain RPC, offered by your node, to prepare the 交易.
3. **Validating the 交易** - You inspect the 交易 and decide whether to sign it.
4. **Signing the 交易** - Once validated, you sign the 交易 hash using your private key (typically with ECDSA/EdDSA).
5. **Submitting the 交易** - You submit the signed 交易 to be executed.
6. **Observing the 交易** - You observe the blockchain until the 交易 is committed.
...
The `execute` 方法 in the `ledger`` namespace will execute the submission and wait for a 响应. THs returns an \`updateId`and`completionOffset\`. Additionally, you can continuously monitor 持仓 changes using token standard history parser.
...
A normal flow on blockchain 应用 is to have dApps that interact with the blockchain on the clients behalf, these flows usually require the 用户 to sign 交易 that the dApp prepares and submit it. To faciliate this in Canton it is required that the prepared 交易 is sent to the 钱包 for signing. An easy way of supporting this is to expose a dApp API (OpenRPC spec can be found here: [https://github.com/canton-网络/钱包/blob/main/api-specs/openrpc-dapp-api.json](https://github.com/canton-网络/钱包/blob/main/api-specs/openrpc-dapp-api.json) ).
...
This is the default config that can be used in combination with a non-altered [Localnet](https://docs.global.canton.网络.sync.global/app_dev/测试/localnet.html) running instance.
...
* **类型 Safety**: The `registerPlugins` 方法 provides full type safety, ensuring that registered plugins are accessible with proper autocompletion and type checking.
...
* **Multiple Plugins**: 你可以 register multiple plugins at once by passing them in a single object to `registerPlugins`.
...
* The current amount of CC awarded to 应用 can be seen in the 'Canton Coin（CC） Reward Split By 角色 Over Time' chart [here](https://canton.thetie.io/).
...
* By qualifying as a Featured 应用 (apply [here](https://sync.global/featured-app-请求/)) and applying a `FeaturedAppActivityMarker` to a 交易 it is marked and converted to reward coupons that can be redeemed.
* A weighting is applied to each 交易 in that Canton Coin（CC） minting round.
...
* 请求 featured 应用 status. Apply [here](https://sync.global/featured-app-请求/).
...
* The 合约 remains active for approximately **20 minutes** (the current round + overlap).
...
1. **用户 Signs Instruction:** The 用户 signs a 交易 to create a `Splice.ExternalPartyAmuletRules.TransferCommand` 合约.
   * This 合约 **does not** reference a mining round.
   * It can remain valid for up to 24 hours (or as defined by `expiresAt`).
2. **Delegated Execution:** Once this 命令 is on the ledger, a 超级验证者 (SV) or a delegate picks it up.
3. **Execution:** The delegate executes the actual transfer. The *delegate* selects the current `OpenMiningRound` at the moment of execution, ensuring the 交易 succeeds regardless of how long ago the 用户 signed the instruction.
...
To enable deposits to be sent to specific 用户 账户 at exchanges, an 账户 identifier needs to be sent to the exchange along with the transfer information. In Canton, the [“memo tag” pattern](https://support.ledger.com/article/4409603715217-zd) is implemented as follows.
...
<img src="https://mintlify.s3.us-west-1.amazonaws.com/cantonfoundation/集成/images/splice-wallet.png" alt="Splice 钱包 UI" />
...
Token standard compliant registries must ensure that they pass the `转账` specification unchanged along when implementing multi-step transfers using the `TransferInstruction` interface ([code](https://github.com/hyperledger-labs/splice/blob/332e06a7ae9e13fde5bba0bf7dcb059aa36f979e/token-standard/splice-api-token-transfer-instruction-v1/daml/Splice/Api/Token/TransferInstructionV1.daml#L108-L115)).
...
USDC on Canton Network represents USDC locked by Circle on the original L1 chain. And this token on Canton Network, is in the form of a Canton Network Standard Token ([CIP-56](https://github.com/canton-foundation/cips/blob/main/cip-0056/cip-0056.md)) as defined through the Canton Network Utilities 服务.

钱包 提供方 and exchanges have three options for supporting USDCx on the Canton Network:

1. 转账 & hold USDCx - Since USDCx is a token standard ([CIP-56](https://github.com/canton-foundation/cips/blob/main/cip-0056/cip-0056.md)) compliant asset then as such any 钱包 that supports the token standard will have built in support for transfers and holding.

2. Support xReserves deposits and withdrawals - Custom API 集成 is required for 钱包 提供方 and exchanges to support xReserve deposits and withdrawals to the utility-bridge daml models to / from their Party using the [xReserves UI](https://digital-asset.github.io/xreserve-deposits/). Instructions for doing this are included in the section “Supporting xReserve 充值 and 提现” below.

3. Integrating the xReserve UI (Ethereum) into the 钱包 - To enable a full end-to-end experience for the 用户, a 钱包 can integrate against Ethereum directly for deposits on top of integrating point 2. To provide an example for doing this, the [xReserves UI](https://digital-asset.github.io/xreserve-deposits/) as well as [open-sourced example scripts](https://github.com/digital-asset/xreserve-deposits) are available for reference. This demonstrates the 2 ethereum 交易 that must be submitted to an ethereum node:
