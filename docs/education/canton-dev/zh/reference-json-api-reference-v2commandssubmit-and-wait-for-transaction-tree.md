...
> 提交一批命令并等待事务树响应。为了向后兼容，它将在 Canton 3.5.0 版本中删除，改用 Submit-and-wait-for-transaction。
...
安全：[]
路径：
  /v2/commands/提交并等待事务树：
    帖子：
      摘要：/v2/commands/submit-and-wait-for-transaction-tree
      描述：>-
        提交一批命令并等待事务树响应。
        提供向后兼容性，它将在 Canton 中删除
        版本 3.5.0，改用提交并等待交易。
      操作 ID：postV2Commands提交并等待交易树
      请求正文：
        内容：
          应用程序/json：
            架构：
  ...
  模式：
    Js命令：
      ...
      属性：
        ...
        命令ID：
          描述：>-
            唯一标识该命令。三元组(user_id, act_as, command_id)构成变更ID
            对于预期的账本变更，
    ...
    JsSubmitAndWaitForTransactionTreeResponse：
      标题：JsSubmitAndWaitForTransactionTreeResponse
      描述：>-
        提供向后兼容性，它将在 Canton 中删除
        版本 3.5.0。
      类型：对象
      ...
      属性：
        ...
        跟踪上下文：
          ...
          描述：>-
            ...
            该消息中传输的跟踪上下文对应于
            提供跟踪上下文由客户端应用程序在原始命令的 HTTP2 标头中执行
            提交。
