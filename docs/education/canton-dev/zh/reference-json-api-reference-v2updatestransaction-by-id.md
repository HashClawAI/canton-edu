...
> 通过id获取交易。为了向后兼容，它将在 Canton 版本 3.5.0 中删除，使用 v2/updates/update-by-id 代替。
...
路径：
  /v2/updates/transaction-by-id:
    帖子：
      摘要：/v2/updates/transaction-by-id
      描述：>-
        通过id获取交易。提供向后兼容性，它将是
        Canton版本3.5.0中删除，使用v2/updates/update-by-id
        相反。
      操作 ID：postV2UpdatesTransaction-by-id
      请求正文：
        内容：
          应用程序/json：
            架构：
      ...
      属性：
        ...
        请求方：
          描述：>-
            ...
            GetTransactionById 请求的向后兼容性可选：
            如果定义的 transaction_format 必须是
        ...
        交易格式：
          ...
          描述：>-
            对于向后兼容的 GetTransactionById 请求是可选的：
            默认为 transaction_format，其中：
