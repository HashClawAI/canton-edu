---
title: "将 Quickstart 部署到 DevNet"
slug: "appdev-quickstart-deploy-to-devnet"
locale: "zh"
category: "appdev"
source_url: "https://docs.canton.network/appdev/quickstart/deploy-to-devnet.md"
source_title: "Deploy Quickstart to DevNet"
tags:
  - appdev
  - quickstart
  - deploy-to-devnet
...
# 将 Quickstart 部署到 DevNet

> 将 Quickstart 应用从 LocalNet 部署到 DevNet，含验证者申请、VPN 配置与端到端工作流验证。

# 部署 Quickstart 到 DevNet
...
| 变量 | LocalNet 值 | DevNet 值 |
| ------------- | -------------- | --------------------------- |
| `LEDGER_HOST` | `localhost` | `grpc-ledger-api.localhost` |
| `LEDGER_PORT` | `5001` | `80` 或 `8080` |
...
## 进入验证者 Docker Compose 目录

```bash theme={"theme":{"light":"github-light","dark":"github-dark"}}
cd splice-node/docker-compose/validator
```

## 连接 Canton Network 验证者节点
...
### 获取 DevNet migration ID 与 Splice 版本
...
默认 party hint 为 `quickstart-USERNAME-1`

回到 `validator` 目录终端，设置 `PARTY_HINT`。
...
<img src="https://mintcdn.com/cantonfoundation/zmlOjLpKuDjnaObr/images/quickstart-devnet/devnet-app-provider-localhost-login.png?fit=max&auto=format&n=zmlOjLpKuDjnaObr&q=85&s=3a6e8bb9c79fc7b98461cdd4b286ca14" width="400" alt="选择 AppProvider" data-path="images/quickstart-devnet/devnet-app-provider-localhost-login.png" />

以 `app-provider` / 密码 `abc123` 登录
...
获取 OAuth2 `AppProvider realm` 令牌：
...
检查 `splice-validator-nginx` 所在网络：
...
#### splice-onboarding 连接问题

若 `splice-onboarding` 日志显示：
