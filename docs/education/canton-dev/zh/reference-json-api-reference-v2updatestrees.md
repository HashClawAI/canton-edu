...
> 查询更新交易树列表（阻塞调用）。为了向后兼容，它将在 Canton 版本 3.5.0 中删除，改用 v2/updates。
注意：此端点应用于小结果集。
    ...
    Js事务树：
      ...
      属性：
        ...
        偏移量：
          描述：>-
            绝对偏移量。该字段的详细信息描述于
            ``community/ledger-api/README.md``。必填，它是一个有效的绝对偏移量（正整数）。
          类型：整数
          格式：int64
        eventsById:
