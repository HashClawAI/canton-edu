...
5. Create a custom Authorization server per validator/environment
   ...
   a. Navigate to "Admin Panel > Directory > Groups", click "Add Group" and name the group "canton\_wallet\_admin".
   b. Add another group "canton\_all."
...
2. Create a client for the Ledger API
   ...
   i. To add this scope to the `ledger-api` client, navigate to "Admin Panel > Clients > ledger\_api > Client scopes" and click "Add client scope". Select `daml_ledger_api` and click "Add" with the type "Default".
